...
root.title('stream-overlay' + version)
...
def find_frame(cx, cy, frame):
    best_d = [10000, 'NaN']
    for keyPressed, value in frame_points.items():
...
print('Custom Stream Overlay ' + version)
print(
    '-----------------------------------------------------------------------------------------------------------------')
print(
    'Disclaimer: There is a high probability you will experience some bugs or that the program will now work at all.\n'
    'This program will also most likely not work on resolutions where the height is bigger than the width and there\n'
    'is no support for custom in-game resolutions or sensitivities. You have been warned.')
print(
    '-----------------------------------------------------------------------------------------------------------------')
...
# preload all images
hit_images = {
    1: PIL.Image.open("images/KeyTapHand1.png"),
    2: PIL.Image.open("images/KeyTapHand2.png")
}

cursor_images = {}
for key in frame_points.keys():
    cursor_images[key] = PIL.Image.open("images/{0}/{1}/Hand {2}.png".format(cursor_device, background, key))
